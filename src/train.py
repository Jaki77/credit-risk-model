...
try:
    ...
    logger.warning(
        f"Could not reach MLflow server at {remote_uri}: {e}. Falling back to local file backend at {local_mlruns}"
    )
...
class ModelTrainer:
    ...
    def load_data(
        self, features_path: str, target_path: str
    ...
    def split_data(
        ...
        logger.info(
            f"Class distribution - Train: {y_train.value_counts(normalize=True).to_dict()}"
        )
    ...
    def train_all_models(
        ...
        for model_name, model_config in models.items():
            try:
                ...
                logger.info(
                    f"✅ {model_name}: AUC={metrics['roc_auc']:.4f}, F1={metrics['f1']:.4f}"
                )
    ...
    def evaluate_on_test(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict:
        ...
        logger.info(
            f"Test Metrics - AUC: {test_metrics['roc_auc']:.4f}, "
    ...
    def plot_confusion_matrix(
        self, y_true: pd.Series, y_pred: pd.Series, model_name: str
    ):
    ...
    def plot_roc_curve(self, y_true: pd.Series, y_proba: np.ndarray, model_name: str):
...
def main():
    ...
    X, y = trainer.load_data(
        "./data/processed/features.csv", "./data/processed/target.csv"
    )
