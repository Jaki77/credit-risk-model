...
class CustomerData(BaseModel):
    ...
    monthly_income: Optional[float] = Field(
        None, ge=0, description="Monthly income (optional)"
    )
...
class PredictionResponse(BaseModel):
    ...
    risk_probability: float = Field(
        ..., ge=0, le=1, description="Probability of high risk (0-1)"
    )
    ...
    approval_recommendation: ApprovalRecommendation = Field(
        ..., description="Loan approval recommendation"
    )
    loan_recommendation: LoanRecommendation = Field(
        ..., description="Loan recommendation details"
    )
