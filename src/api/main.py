...
def get_predictor():
    ...
    if predictor is None:
        try:
            ...
            processor_path = os.getenv("PROCESSOR_PATH", "../data/processed/processor.joblib")
...
async def health_check():
    ...
    try:
        ...
        return HealthCheck(
            ...
            version="1.0.0",
            model_loaded=True,
            model_name=pred.model_info.get('model_name', 'unknown') if pred.model_info else 'unknown'
        )
    except Exception as e:
...
async def predict(request: PredictionRequest, predictor: CreditRiskPredictor = Depends(get_predictor)):
    """
    ...
    try:
        ...
        logger.info(f"Prediction completed for {request.customer_id}: "
...
async def batch_predict(
    ...
    try:
        ...
        logger.info(f"Batch prediction completed: {response.high_risk_count} high risk customers")
