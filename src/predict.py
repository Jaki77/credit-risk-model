...
class CreditRiskPredictor:
    ...
    def load_model(self, model_path: str):
        ...
        try:
            ...
            logger.info(
                f"Loaded model info: {self.model_info.get('model_name', 'Unknown')}"
            )
        except:
    ...
    def prepare_features(
        self, customer_data: pd.DataFrame, transaction_data: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
        ...
        if transaction_data is not None and self.processor is not None:
            ...
            features = rfm_features.merge(
                aggregate_features, on="CustomerId", how="left"
            )
    ...
    def predict_risk_probability(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Predictions completed. High risk: {results['prediction'].sum()}"
            )
            return results
    ...
    def predict_loan_terms(
        ...
        recommendation = {
            ...
            "debt_to_income_ratio": (
                round(monthly_payment / monthly_income * 100, 2)
                if monthly_income
                else None
            ),
            "approval_recommendation": (
                "APPROVE" if risk_probability < 0.5 else "REVIEW"
            ),
    ...
    def batch_predict(
        ...
        for _, row in predictions.iterrows():
            recommendation = self.predict_loan_terms(
                customer_id=row["CustomerId"], risk_probability=row["risk_probability"]
            )
