...
class DataProcessor:
        ...
        if "TransactionStartTime" in df.columns:
            ...
            try:
                ...
                df["TransactionStartTime"] = df["TransactionStartTime"].apply(
                    lambda ts: (
                        ts.tz_convert(None)
                        if hasattr(ts, "tz") and ts is not pd.NaT and ts.tz is not None
                        else ts
                    )
                )
    ...
    def create_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "TransactionStartTime" in df.columns:
            ...
            df["TransactionWeekOfYear"] = (
                df["TransactionStartTime"].dt.isocalendar().week
            )
    ...
    def create_behavioral_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_sorted["TimeSinceLast"] = (
            df_sorted.groupby("CustomerId")["TransactionStartTime"]
            .diff()
            .dt.total_seconds()
            / 3600
        )  # hours
        ...
        behavioral_agg = (
            df_sorted.groupby("CustomerId")
            .agg(
                {
                    "TimeSinceLast": ["mean", "std", "min", "max"],
                    "ProductCategory": lambda x: (
                        x.nunique() if "ProductCategory" in df.columns else 0
                    ),
                    "ProviderId": lambda x: (
                        x.nunique() if "ProviderId" in df.columns else 0
                    ),
                    "ChannelId": lambda x: (
                        x.nunique() if "ChannelId" in df.columns else 0
                    ),
                }
            )
            .reset_index()
        ...
        behavioral_agg.columns = ["CustomerId"] + [
            (
                f"Behavior_{col[0]}_{col[1]}"
                if col[1] != "<lambda>"
                else f"Behavior_{col[0]}_nunique"
            )
    ...
    def identify_high_risk_clusters(
        self, rfm_df: pd.DataFrame, n_clusters: int = 3
    ) -> pd.DataFrame:
        """
    ...
    def prepare_training_data(
        self, df: pd.DataFrame, target_df: pd.DataFrame
        ...
        # Merge with target
        X = X.merge(
            target_df[["CustomerId", "is_high_risk"]], on="CustomerId", how="inner"
        )
